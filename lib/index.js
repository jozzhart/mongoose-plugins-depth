...
var findExpand = function (query, depth, fieldsFilter, cb) {
  if(_.isUndefined(this.schema.filter)) return cb('Model ' + this.modelName + ' is missing a filter');

  // If fields filter is empty, use model default
  if(fieldsFilter.length === 0)
    fieldsFilter = this.schema.filter;
  ...
  this.find(query).lean().exec(function(err, results) {
    ...
    if(depth > 0 && expandFields.length > 0) {
      queryChildren(results, depth, fieldsFilter, expandFields, schemaFields, function(err, results){
        cb(null, filterResults(results, currentFields));  
      });
    } else {
      cb(null, filterResults(results, currentFields));  
    }
...
var queryChildren = function(results, depth, fieldsFilter, expandFields, parentSchema, callback) {
  async.map(results, function(result, callback){
    async.each(expandFields, function(field, callback){
      if(_.isUndefined(mongoose.model(field.model).schema.filter)) return callback('Model ' + field.model + ' is missing a filter');

      //  Get child fields of current Model field, and default filter for Model
      var childFields = getChildFields(fieldsFilter, field.field);
      if(childFields.length === 0) childFields = mongoose.model(field.model).schema.filter;
      
      mongoose.model(field.model).findExpand({ _id : result[field.field] }, depth - 1, childFields, function(err, instances){

        //  If there is filtering passed in, filter, otherise return the default filtering for the model
        if(childFields.length === 0) childFields = defaultFilter;               
...
var filterResults = function(results, filterFields) {
  ...
  //console.log(results, filterFields);

  return _.map(results, function(result){
    if(result.id == null) 
      result.id = result._id;

    console.log('poick', _.pick(result, filterFields));
