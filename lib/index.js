...
var findFP = function (query, opts, cb) {
  ...
  // //  Check for passed in include/exlude fi
  // if(!_.isUndefined(opts.excludeFields)) {
  //   opts.depth = _getFilterDepth(opts.excludeFields);
  // }
  ...
  this.find(query, null, opts.paging).lean().exec(function(err, results) {
    ...
    if(opts.depth > 1) {
      _queryChildren(results, _.clone(opts), function(err, results){
        ...
        if(_.isUndefined(opts.isChild) && !_.isUndefined(opts.hasFiltering)) return cb(err, _rootFilter(results, opts));
...
var findByIdAndUpdateFP = function (id, body, opts, callback) {
  ...
  model.findByIdAndUpdate(id, body, function(err, result){
    ...
    model.findFP({ _id: result.id }, opts, function(err, results){
      if(results == null) return callback(err, null)
...
var saveFP = function(opts, callback) {
  ...
  .then(function(instance) {
    _recursiveFindFP(opts, model, instance, callback)
  })
  .catch(function(err) {
    callback (err, null);
  });
...
var _recursiveFindFP = function(opts, model, instance, callback) {

  mongoose.model(model.constructor.modelName)
  .findFP({ _id : instance._id}, opts, function(err, results){
    if(err) return callback(err);

    if(results) {
      callback(null, results[0])
    } else {
      return _recursiveFindFP(opts, model, instance, callback)
    }
  });
};
...
var _queryChild = function(result, opts, callback) {
  ...
  async.each(opts.populateFields, function(field, callback){
    ...
    // If document has reference to Model that no longer exists
    try {
      mongoose.model(field.model)
    } catch(err) {
    ...
    mongoose.model(field.model).findFP(query, _.clone(options), function(err, instances){
      ...
      if (_.isArray(result[field.field])) {
        instances = _.map(result[field.field], function(item) {
          return _.find(instances, function(instance) {
            if(!instance || !instance.id) return false;
            return instance.id.toString() === item.toString();
          }) || {};
        });
      }
...
var _rootFilter = function(results, opts) {
  ...
  if(opts.excludeFields) {
    return _.map(results, function(result) {
      var schemaFilter = opts.schemaFilter;
      return _.pick(stepThroughFilterExclude([result], opts.excludeFields)[0], schemaFilter);
    });
  } else {
    return _.map(results, function(result) {
      return stepThroughFilter([result], opts.includeFields)[0];
    });
  }
}

var stepThroughFilter = function(results, currentFields) {
...
var stepThroughFilterExclude = function(results, currentFields) {
  ...
  return _.map(results, function(result){
    ...
    _.each(filterFields, function(field){
      ...
      if(childFields.length > 0 && !_.isUndefined(result[field])) {
        if(_.isArray(result[field]))
          result[field] = stepThroughFilterExclude(result[field], childFields);
        else
          result[field] = stepThroughFilterExclude([result[field]], childFields)[0];
      }
    });

    //  Check if it is an unpopluated field, resulting from relationship to entity no longer existing
    if(/^[0-9a-fA-F]{24}$/.test(result.toString())) return {};
    return _.omit(result, _getCurrentFieldsExclude(currentFields));
  });
};
...
var _getCurrentFieldsExclude = function(dotNotionFields) {

  return _.uniq(_.compact(_.filter(dotNotionFields, function(field){
