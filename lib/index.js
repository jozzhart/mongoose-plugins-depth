...
var findOne = function (query, opts, cb) {
  ...
  this.findOne(query).lean().exec(function(err, result) {
    ...
    if(result == null || result.length === 0) return cb(null);

    //  Our recursion check point
    if(opts.depth > 0 && populateFields.length > 0) {

      _queryChildren([result], opts.depth, opts.filterFields, populateFields, schemaFields, function(err, results){
        cb(err, _filterResults(results, currentFields)[0]);  
      });
    } else {
