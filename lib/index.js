...
var find = function (query, opts, cb) {
...
var findOne = function (query, opts, cb) {
...
var findByIdAndUpdate = function (id, body, opts, cb) {

  // this that for the moment
  var that = this;
  
  this.findByIdAndUpdate(id, body, function(err, result){
    if (err) return callback (err, null);
    if (result === null) return callback (null, null);
    //  Filter and Populate
    that.findFP({ _id: result.id }, opts, function(err, results){
      if(results == null) return cb(err, null)
      cb(err, results[0]);
    });
  });
...
var save = function(opts, callback) {
  ...
  var filterFields = this.schema.filterFields;

  this.save (function (err, instance) {
    if (err) return callback (err, null);
    return callback (null, _filterResult(instance.toObject(), filterFields));
  });
}
...
module.exports = exports = function mongooseExpandPlugin(schema, options) {
  ...
  schema.statics.findFP = find;
  schema.statics.findOneFP = findOne;
  schema.statics.findByIdAndUpdateFP = findByIdAndUpdate;
  schema.methods.saveFP = save;
