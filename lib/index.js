...
var _rootFilter = function(results, opts) {
  ...
  if(opts.excludeFields) {
    return _.map(results, function(result) {
      return stepThroughFilterExclude([result], opts.excludeFields, opts.schemaFilter)[0];
    });
  } else {
    return _.map(results, function(result) {
...
var stepThroughFilterExclude = function(results, currentFields, schemaFields) {
  ...
  return _.map(results, function(result){
    ...
    return _.pick(_.omit(_.clone(result), _getCurrentFieldsExclude(currentFields)), schemaFields);
