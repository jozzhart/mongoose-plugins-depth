...
var findFP = function (query, opts, cb) {
  ...
  if(!_.isUndefined(opts.filterFields)) {
    opts.depth = _getFilterDepth(opts.filterFields);
  }
  ...
  this.find(query).lean().exec(function(err, results) {
    ...
    if(opts.depth > 1) {
      _queryChildren(results, _.clone(opts), function(err, results){
        ...
        if(_.isUndefined(opts.isChild) && !_.isUndefined(opts.filterFields)) return cb(err, _rootFilter(results, opts));    
...
var _rootFilter = function(results, opts) {
  ...
  return _.map(results, function(result) {
    return stepThroughFilter([result], opts.filterFields, opts.schemaFilter)[0];
  });
...
var _getFilterDepth = function(filterFields) {
  ...
  if(filterFields.indexOf('*')) return 10;
